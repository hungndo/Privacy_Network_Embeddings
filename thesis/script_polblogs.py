"""
DP-SBM comparison experiment on the polblogs real-world dataset.

Identical to script_no_known_node_label_synthetic_graph.py in every
algorithmic detail -- same two methods, same accountant, same forward
sigma->epsilon matching -- except:

  * Data: largest connected component of polblogs-1.gml (fixed graph,
    loaded once).  Ground-truth labels come from the "value" node
    attribute (0 = liberal, 1 = conservative blog).

  * No N sweep: N is fixed at the LCC size.  The sigma sweep (SIGMAS)
    is unchanged.

  * "True" betas: since there are no planted parameters, we compute
    empirical block-pair edge densities from the ground-truth labels
    and use those as the reference (beta_true_*).

  * Reps: all N_REPS repetitions run on the SAME adjacency matrix
    (real data -- we can't resample it).  Different rep seeds control
    the DP-mechanism randomness only.
"""
...
def binary_sbm_estimate_fully_variational(
    ...
    for step in range(iter):
        ...
        for gamma_step in range(gamma_steps):
            ...
            # reference cycle that gc.collect can't break -- one ~13 GB graph
            # leaks per rep. Detaching keeps identical values, drops the graph.
...
def empirical_betas(A_np, labels, K=2):
    """
    Compute block-pair edge densities from ground-truth labels.
    Returns (beta_00, beta_01, beta_11).
    """
    betas = {}
    for k in range(K):
        for l in range(k, K):
...
def run(shard_id=0, num_shards=1):
    ...
    # Load data once -- all reps share the same fixed graph
    ...
    print(f"  N={N}  edges={int(A_np.sum() / 2)}", flush=True)
    print(f"  Empirical betas (from ground truth): "
          f"beta_00={true_00:.4f}  beta_01={true_01:.4f}  beta_11={true_11:.4f}", flush=True)
    ...
    with open(output_path, "a", newline="") as f:
        ...
        for sigma in get_sharded_grid(shard_id, num_shards):
            ...
            print(f"[sigma={sigma}] -> epsilon={epsilon:.4f} "
                  f"(eps_gamma={eps_gamma:.4f} eps_beta={eps_beta:.4f} eps_rho={eps_rho:.4f})", flush=True)

            for rep in range(N_REPS):
                rep_start = time.time()
                # seed controls DP randomness only (graph is fixed)
                seed = hash(("polblogs", rep)) % (2**31)
                ...
                print(f"  sigma={sigma} rep {rep + 1}/{N_REPS} done in {rep_secs:.1f}s  "
                      f"[cur RSS: {cur_rss:.1f} GB | peak RSS: {peak_rss:.1f} GB]", flush=True)
