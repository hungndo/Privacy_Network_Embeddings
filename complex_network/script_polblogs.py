"""
DP-SBM comparison experiment on polblogs (complex-network variant).

Derived from thesis/script_polblogs.py, carrying the SAME five changes that
complex_network/script_no_known_node_label_synthetic_graph.py carries, with
the same [CHANGE n] numbering so the two files cross-reference.  Read that
file's docstring for the full rationale; only what differs is repeated here.

  [CHANGE 1] Analytic per-example gradient wrt gamma_logits, replacing the
      vmap(grad(...)) path.  Motivated there by N=10000; it matters here too
      -- polblogs' LCC is N=1222, so at sample_pct=0.1 the old path
      materialized an (L, N, K) tensor of 149,206 x 1222 x 2 floats = 1.46 GB
      per gamma step.  (That is also the scale behind the ~13 GB per-rep
      autograd-graph leak the thesis script's detach comment describes.)

  [CHANGE 2] sigma grid [1.0, 2.0, 5.0, 10.0, 20.0]: the smallest sigma
      dropped (epsilon ~ 1600, and np.exp overflows float64 above ~709.8
      inside edge_flip's p_flip), a larger sigma added at the private end.
  ...
  [CHANGE 4] New method `edge_flip_spectral`, sharing ONE flipped release
      per (sigma, rep) with edge_flip_vem -- two independent flips would
      compose to 2*epsilon.
...
POLBLOGS-SPECIFIC, unchanged from thesis/script_polblogs.py:

  * Data: largest connected component of polblogs-1.gml (fixed graph, loaded
    once).  N = 1222.  Ground-truth labels come from the "value" node
    attribute (0 = liberal, 1 = conservative blog).

  * No N sweep -- N is fixed at the LCC size, so unlike the synthetic driver
    there is no GRAPH_SIZES axis and no per-rep graph draw.

  * "True" betas: there are no planted parameters, so beta_true_* are the
    empirical block-pair edge densities computed from the ground-truth
    labels on the UNPERTURBED adjacency.

  * Reps: all N_REPS repetitions run on the SAME adjacency (real data --
    it cannot be resampled).  Rep seeds control DP-mechanism randomness
    only, so rep variance here is mechanism variance alone, NOT the
    graph-plus-mechanism variance the synthetic driver reports.  The two
    scripts' error bars are therefore not directly comparable.

REPRODUCIBILITY FIX vs the thesis script: it seeded reps with
`hash(("polblogs", rep))`.  Python randomizes str hashing per process
...
# The gml lives in thesis/; it is not duplicated here.  Searched in order so
# the script runs both from this directory and from the repo root.
DATA_PATH_CANDIDATES = [
    "./polblogs-1.gml",
    "../thesis/polblogs-1.gml",
    "./thesis/polblogs-1.gml",
]

# [CHANGE 2] 0.1 and 0.5 dropped, 20.0 added.  Budgets are roughly
# epsilon = 27, 12, 4.1, 2.0, 1.0 (N-independent up to flooring in q).
...
# so the two experiments differ only in their data.  See that file's comment
# block for what these values cost under [CHANGE 3] and why lr_gamma was
# moved off the thesis value of 3; tune_sbm_dpsgd.ipynb explores alternatives.
#
# NOTE: that tuning evidence was gathered on SYNTHETIC graphs (N=500,
# planted p=0.2/q=0.02).  polblogs is a real graph -- N=1222, mean degree
# ~27, heavy-tailed, and only approximately 2-block -- so there is no
# guarantee these transfer.  Re-check here before trusting them.
...
OUTPUT_CSV = "results_dp_sbm_polblogs.csv"
...
def resolve_data_path(candidates=None):
    """First existing path in `candidates`, defaulting to the module-level
    DATA_PATH_CANDIDATES.  Read at CALL time, not bound as a default
    argument, so overriding the module global actually takes effect."""
    candidates = DATA_PATH_CANDIDATES if candidates is None else candidates
    for p in candidates:
        if os.path.exists(p):
            return p
    raise FileNotFoundError(
        "polblogs-1.gml not found. Looked in:\n  "
        + "\n  ".join(os.path.abspath(p) for p in candidates)
        + f"\nRun from {os.path.dirname(os.path.abspath(__file__))} or the repo "
          "root, or add the path to DATA_PATH_CANDIDATES."
    )


def load_polblogs(path):
    """
    Load polblogs GML, restrict to the largest connected component, and
    return (A_np, labels) where labels are the ground-truth "value"
    attributes (0 = liberal, 1 = conservative).
    """
    G = nx.read_gml(path)
    G = nx.Graph(G)
    G = G.to_undirected()
    ...
    labels = np.array([data["value"] for _, data in G_lcc.nodes(data=True)])
    return A_np, labels


def empirical_betas(A_np, labels, K=2):
    """
    Block-pair edge densities from the ground-truth labels on the
    UNPERTURBED adjacency -- polblogs has no planted parameters, so these
    stand in for beta_true_*.  Returns (beta_00, beta_01, beta_11).
    """
    dens = block_densities(A_np, labels, K)
    return dens[0, 0], dens[0, 1], dens[1, 1]


def rep_seed(rep):
    """
    Deterministic per-rep seed.

    The thesis script used `hash(("polblogs", rep))`, but Python randomizes
    str hashing per process (PYTHONHASHSEED), so those seeds changed on every
    run and differed across shards -- runs were not reproducible.  A fixed
    base keeps reps distinct AND stable.
    """
    return (982_451_653 + 7_919 * rep) % (2 ** 31)


def get_sharded_grid(shard_id, num_shards):
    """
    Shard over (sigma, rep).  The graph is fixed and loaded once, so unlike
    the synthetic driver there is no graph-redraw cost to avoid -- this is
    purely to give SLURM arrays a finer unit than one-shard-per-sigma.
    """
    full_grid = [(sigma, rep) for sigma in SIGMAS for rep in range(N_REPS)]
    if num_shards <= 1:
        return full_grid
...
def run(shard_id=0, num_shards=1):
    ...
    # Load data once -- every rep shares this fixed graph.
    data_path = resolve_data_path()
    print(f"Loading polblogs from {data_path} ...", flush=True)
    A_np, labels = load_polblogs(data_path)
    N = A_np.shape[0]
    A_t = torch.tensor(A_np, dtype=torch.float32, device=DEVICE)
    true_00, true_01, true_11 = empirical_betas(A_np, labels)
    print(f"  N={N}  edges={int(A_np.sum() / 2)}  "
          f"block sizes={np.bincount(labels).tolist()}", flush=True)
    print(f"  Empirical betas (from ground truth): "
          f"beta_00={true_00:.4f}  beta_01={true_01:.4f}  beta_11={true_11:.4f}",
          flush=True)
    ...
    with open(output_path, "a", newline="") as f:
        ...
        for sigma, rep in get_sharded_grid(shard_id, num_shards):
            rep_start = time.time()
            ...
            # Sharing the release keeps both methods at the same epsilon the
            # SBM accountant produced.
            ...
            nmi_ef = normalized_mutual_info_score(labels, labels_ef)
            write_row(
            ...
            nmi_sc = normalized_mutual_info_score(labels, labels_sc)
            write_row(
                writer, N=N, sigma=sigma, epsilon=epsilon, rep=rep,
            ...
            nmi_sbm = normalized_mutual_info_score(labels, labels_sbm)
            write_row(
                writer, N=N, sigma=sigma, epsilon=epsilon, rep=rep,
            ...
            print(f"  sigma={sigma} rep={rep} (eps={epsilon:.3f}) "
