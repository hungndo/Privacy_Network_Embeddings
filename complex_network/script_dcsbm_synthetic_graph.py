"""
DP-SBM comparison experiment on DEGREE-CORRECTED synthetic graphs
(complex-network variant), n = 500.

Derived from thesis/script_dcsbm_synthetic_graph.py, carrying the SAME five
...
Read that file's docstring for the full rationale.
  ...
  [CHANGE 2] sigma grid is now [1.0, 2.0, 5.0, 10.0, 20.0] -- 0.5 dropped
      from the thesis grid, 20.0 added at the private end.
  ...
  [CHANGE 4] New method `edge_flip_spectral`, sharing ONE flipped release
      per (rep, sigma) with edge_flip_vem -- two independent flips would
      compose to 2*epsilon.  Worth watching on THIS generator in
      particular: spectral clustering on a raw adjacency is well known to
      be pulled around by degree heterogeneity, which is exactly what the
      DC-SBM introduces, so a gap between edge_flip_spectral here and on
      the plain-SBM driver is a degree-correction effect rather than a
      privacy one.
...
DC-SBM-SPECIFIC, unchanged from the thesis script:

  * Data: degree-corrected SBM following Gao et al. (2018) as used by
    Davison, Morgan & Ward (arXiv:2310.17712):
  ...
  * DENSE parameters, matching the plain-SBM experiment rather than the
    sparse one: p = 0.2, q = 0.02.  Mean degree lands around 55, so graphs
    are connected with overwhelming probability -- none of the connectivity
    trouble from the sparse driver applies, and no LCC extraction is done.
  ...
  * NOTE ON E[theta], AND WHY NORMALIZE_WEIGHTS DEFAULTS TO True.
    ...
    This keeps p, q comparable to the plain-SBM experiment, so a difference
    in results is attributable to degree correction rather than to the DC
    graphs being 36% sparser.  Set it back to False to reproduce the
    reference code verbatim.

  * BETA REFERENCE.  `beta_true_*` is the EMPIRICAL block-pair edge density
    of each drawn graph -- the quantity a plain-SBM estimator actually
    targets (VEM's M-step is literally beta = gamma^T A gamma / denom) --
    matching how script_polblogs.py handles real data.  The generative
    constants are recorded separately as `beta_nominal_*`.  With
    NORMALIZE_WEIGHTS = True the two agree to within ~0.007% in
    expectation; it matters a great deal if it is turned off, where scoring
    against nominal would show a systematic ~35% relative error that is
    purely the centering constant, not estimator bias.

REPRODUCIBILITY FIX vs the thesis script: it seeded reps with
`hash(("dcsbm", rep))`.  Python randomizes str hashing per process
...
GRAPH_SIZE = 500       # TOTAL nodes (so with K=2 each community has 250 --
                       # matching how the other scripts define N).
NUM_BLOCKS = 2

TRUE_P = 0.2           # within-block probability (dense, matching the
TRUE_R = 0.02          # between-block probability   plain-SBM experiment)
...
# make_dcsbm_graph, which silently compresses the heterogeneity being added.
DC_SIGMA = 0.25
...
# heterogeneity is fully preserved.  See the module docstring for the
# derivation.
...
# alternatives.  NOTE: that tuning was done on a PLAIN SBM; degree correction
# changes the gradient landscape, so re-check before trusting them here.
...
def make_dcsbm_graph(N, K, p, q, seed, dc_sigma=DC_SIGMA, normalize=NORMALIZE_WEIGHTS):
    ...
    Draw a degree-corrected SBM graph.
        ...
        truth   = balanced blocks
        z       = N(0, dc_sigma) per node
        theta   = |z| + 1 - (2*pi)^(-1/2)
        P(edge) = theta_i * theta_j * P[c_i, c_j]

    The reference draws each pair in a Python double loop; this draws the
    upper triangle vectorized -- same distribution, much faster.

    Returns (A, labels, weights).  The graph is returned AS DRAWN: no
    connectivity check and no LCC extraction.  At mean degree ~55 that is
    ...
    # (With p=0.2 and dc_sigma=0.25 the realized max is ~0.38, so this never
    # bites -- it is here so the generator stays valid if p is raised.)
...
def rep_seed(rep):
    ...
    The thesis script used `hash(("dcsbm", rep))`, but Python randomizes str
...
def get_sharded_grid(shard_id, num_shards):
    """
    Shard over rep.  The graph seed depends only on rep, so this ordering
    draws each graph once and sweeps all sigmas against it.
    """
    full_grid = list(range(N_REPS))
    if num_shards <= 1:
        return full_grid
...
def run(shard_id=0, num_shards=1):
    ...
    with open(output_path, "a", newline="") as f:
        ...
        for rep in get_sharded_grid(shard_id, num_shards):
            # Seed depends on rep only, NOT on sigma, so a given rep sees the
            ...
            print(f"[rep={rep}] mean degree {gstats['mean_degree']:.1f} "
                  f"(cv {gstats['degree_cv']:.3f})  weight cv {gstats['weight_cv']:.3f}  "
                  f"empirical betas {true_00:.4f}/{true_01:.4f}/{true_11:.4f}", flush=True)
            ...
            for sigma in SIGMAS:
                ...
                labels_ef, epochs_ef = labels_from_vem(
                    A_flipped_t, K, seed=seed, n_restarts=N_VEM_RESTARTS)
                beta_ef = debiased_block_beta(A_flipped, labels_ef, K, epsilon)
                ...
                labels_sc = labels_from_spectral(A_flipped, K, seed=seed)
                beta_sc = debiased_block_beta(A_flipped, labels_sc, K, epsilon)
                ...
                print(f"  rep={rep} sigma={sigma} (eps={epsilon:.3f}) "
